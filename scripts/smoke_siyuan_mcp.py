...
from mcp_servers.siyuan_server.tools import SiYuanService
...
async def run_smoke(service: SiYuanService, *, timestamp: str | None = None) -> dict[str, str]:
    ...
    path = f"/Growen/Pruebas MCP/{stamp}"
    ...
    if SMOKE_MARKDOWN.strip() not in reread["markdown"]:
...
__all__ = ["SMOKE_MARKDOWN", "run_smoke"]
