...
import type { EnqueueResult, HistoryPoint, MarketFilters, MarketJob, MarketProductPage, MarketSource, ProductSources } from '../types'
...
export async function refreshProduct(productId: number): Promise<EnqueueResult> { return (await http.post<EnqueueResult>(`/market/products/${productId}/refresh-market`)).data }
export async function refreshProducts(productIds: number[]): Promise<{ results: Array<{ product_id: number; job_id: string | null; item_id: number | null; deduplicated: boolean; status: string }> }> { return (await http.post('/market/products/batch-refresh', { product_ids: productIds })).data }
...
export async function discoverMarketSources(productId: number): Promise<Array<{ url: string; title: string; snippet: string }>> { return (await http.post<{ sources: Array<{ url: string; title: string; snippet: string }> }>(`/market/products/${productId}/discover-sources`)).data.sources }
