...
import { createSale, listChannels, quoteSale, searchProducts, uploadAttachment, type SaleDraft, type SaleLine, type CatalogProduct } from '../../../services/sales'
...
const products = ref<CatalogProduct[]>([]); const productSearch = ref(''); const items = ref<SaleLine[]>([]); const channels = ref<any[]>([]); const channelId = ref<number | null>(null)
...
const isCollaborator = computed(() => customer.value?.kind === 'colaborador')
...
watch([items, costs, channelId, saleKind, note, customer], async () => {
  dirty.value = true
  try {
    quote.value = await quoteSale({
      items: items.value,
      additional_costs: costs.value,
      customer_id: customer.value?.id,
      is_collaborator: isCollaborator.value,
    })
  } catch { /* el guardado mostrará el error autoritativo */ }
}, { deep: true })

watch(customer, newVal => {
  if (newVal?.kind === 'colaborador' && items.value.length > 0) {
    items.value.forEach(line => {
      const found = products.value.find(p => p.product_id === line.product_id)
      if (found?.cost_price && found.cost_price > 0) {
        line.unit_price = found.cost_price
      }
    })
  }
})
...
function addProduct(product: CatalogProduct) {
  const current = items.value.find(item => item.product_id === product.product_id)
  if (current) {
    current.qty += 1
  } else {
    let unitPrice = product.price || 0
    if (isCollaborator.value) {
      if (product.cost_price !== null && product.cost_price !== undefined && product.cost_price > 0) {
        unitPrice = product.cost_price
      } else {
        error.value = `El producto "${product.title}" no tiene precio de costo registrado.`
      }
    }
    items.value.push({
...
async function save() {
  ...
  try {
    const payload: SaleDraft = {
      customer: customer.value ? { id: customer.value.id } : undefined,
      customer_id: customer.value?.id,
      is_collaborator: isCollaborator.value,
      items: items.value,
...
<template><v-container fluid class="py-8"><v-btn to="/ventas" variant="text">Volver</v-btn><h1 class="text-h4 mb-6">Nueva venta</h1><v-alert v-if="error" type="error" class="mb-4" closable @click:close="error=''">{{error}}</v-alert><v-row><v-col cols="12" lg="8"><v-card><v-card-text><v-row><v-col><div class="d-flex align-center"><v-autocomplete v-model="customer" v-model:search="customerSearch" :items="customers" item-title="name" return-object label="Cliente (vacío = Consumidor Final)"/><v-chip v-if="isCollaborator" color="purple" class="ml-2 mb-4" size="small" variant="tonal" prepend-icon="mdi-account-star">Colaborador · Precio de costo</v-chip></div><v-btn size="small" variant="text" @click="quickDialog=true">Alta rápida</v-btn></v-col><v-col><v-select v-model="saleKind" :items="['MOSTRADOR','PEDIDO']" label="Tipo"/></v-col><v-col><v-select v-model="channelId" :items="channels" item-title="name" item-value="id" label="Canal" clearable/></v-col></v-row><v-autocomplete v-model:search="productSearch" :items="products" item-title="title" return-object label="Buscar producto, SKU o código" @update:model-value="product=>product&&addProduct(product)"/>
