...
<template><v-container fluid class="py-8">
  ...
  <v-card><v-card-text class="d-flex ga-3 flex-wrap"><v-text-field v-model="q" label="Buscar" prepend-inner-icon="mdi-magnify" hide-details clearable /><v-select v-model="kind" :items="['cf','ri','minorista','mayorista','colaborador']" label="Tipo" hide-details clearable /><v-switch v-model="active" label="Solo activos" hide-details /></v-card-text>
  <v-data-table :headers="[{title:'Nombre',key:'name'},{title:'Documento',key:'document_number'},{title:'Contacto',key:'email'},{title:'Tipo',key:'kind'},{title:'Estado',key:'is_active'},{title:'',key:'actions',sortable:false}]" :items="rows" :loading="loading" hide-default-footer>
    <template #item.kind="{ item }"><v-chip v-if="item.kind==='colaborador'" color="purple" size="small" variant="tonal">Colaborador</v-chip><span v-else>{{ item.kind || '-' }}</span></template>
