...
<!-- NG-HEADER: Ubicación: docs/development/DEVELOPMENT_WORKFLOW.md -->
...
La integración MeLi se prueba con `docker compose --profile meli up -d --build`; no se mezcla con `market_worker`. Requiere migración head, cuatro archivos secretos externos y `MELI_REDIRECT_URI` HTTPS. El gateway no publica puerto local y se alcanza únicamente mediante Cloudflare Tunnel. Para producción multinodo usar `docker-stack.yml` y `scripts/deploy-swarm.ps1`; detalles en `docs/features/MELI_INTEGRATION.md` y `docs/operations/DOCKER_SWARM.md`.
...
- `start-dev.ps1` exporta `GROWEN_DEV_RUN_LOG_DIR` a los procesos hijos. Servicios → Mantenimiento de logs y `scripts/cleanup_logs.py` usan ese dato más los PID de `state.json` para proteger ejecuciones activas y eliminan las ejecuciones antiguas como carpetas completas. Ver `docs/operations/LOG_CLEANUP.md`.
...
- Estado detallado y fases: `docs/development/FRONTEND_MIGRATION_VUE.md`.
