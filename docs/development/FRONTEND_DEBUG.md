...
<!-- NG-HEADER: Ubicación: docs/development/FRONTEND_DEBUG.md -->
...
- Plan y estado: `docs/development/FRONTEND_MIGRATION_VUE.md`.
...
Para diagnosticar qué SPA debe responder una ruta, revisar `frontend-vue/config/modules.json`, regenerar con `npm run generate:nginx` y contrastar `frontend-vue/generated/nginx-spa-routes.conf`. La guía completa está en `docs/development/FRONTEND_MIGRATION_OPERATIONS.md`.
...
- Botón "Reintentar" en la UI de ErrorBoundary fuerza un reload completo.
 - Botón flotante "Reportar" permite enviar un reporte manual a `/bug-report` incluyendo URL y hora (GMT-3). Ver `docs/development/BUG_REPORTS.md`.
