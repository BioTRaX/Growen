...
<!-- NG-HEADER: Ubicación: docs/retrospectives/SECURITY_INCIDENT_TELEGRAM_20260730.md -->
...
Estado: cerrado; credencial revocada y referencias afectadas saneadas

Actualización de cierre: la confirmación operativa recibida informa que la
purga remota terminó y que no quedan acciones pendientes de este incidente.
Esta actualización documental no sustituye una nueva inspección remota.
...
de Telegram en 130 referencias de ramas y tags. En esa auditoría el objeto aún
era alcanzable desde referencias internas `refs/pull/*` administradas por
GitHub. La confirmación operativa posterior registra que esas referencias y sus
objetos asociados ya fueron purgados. La revocación realizada en BotFather fue
la contención correcta y debe considerarse definitiva para esa credencial.
...
Estado: cerrado; token revocado y referencias administrables e internas saneadas
...
La verificación independiente de 2026-07-30 detectó que el blob afectado era
alcanzable desde referencias internas creadas por pull requests:
...
Estas referencias pertenecen a GitHub y no aceptan force-push del propietario.
La confirmación operativa posterior registra que su purga por el proveedor ya
finalizó; la lista se conserva únicamente como evidencia histórica redactada.
...
7. [x] Solicitar a GitHub Support la purga del blob histórico y de las diez
   referencias `refs/pull/*` enumeradas en F-01.
8. [x] Invalidar clones, forks, caches de CI y artefactos; exigir reclonado
...
`docs/retrospectives/RETROSPECTIVE_TELEGRAM_SECRET_FORENSICS_20260815.md`.
...
- [x] Referencias internas `refs/pull/*` y objetos asociados purgados por
  GitHub Support.
- [x] Clones, forks, caches y artefactos invalidados o revisados.
...
- [x] Cambios documentados y cualquier instrucción desactualizada corregida.
