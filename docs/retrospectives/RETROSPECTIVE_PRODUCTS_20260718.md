...
<!-- NG-HEADER: Ubicación: docs/retrospectives/RETROSPECTIVE_PRODUCTS_20260718.md -->
...
- Las pruebas focales de backend y las pruebas PostgreSQL de migración/concurrencia quedaron aprobadas durante la implementación del batch; `docs/features/MIGRATIONS_NOTES.md` conserva el detalle de la cadena temporal y las 12 asignaciones concurrentes verificadas.
...
- La ampliación posterior de taxonomía plana y tags, con métricas de validación actualizadas e incidentes adicionales, se documenta en `docs/retrospectives/RETROSPECTIVE_PRODUCTS_TAXONOMY_TAGS_20260720.md`.
...
Corrección: ejecutar `npm.cmd test -- <rutas>` sin repetir `--run`. La regla quedó incorporada en `docs/development/TESTING.md`.
...
Evidencia: `../features/API_PRODUCTS.md`, `../features/PRODUCTS_UI.md` y `../features/roles-endpoints.md` describían `GET /catalog/next-seq` como preview genérico de “la UI”, mientras Vue ya consumía `/canonical-products/sku-preview`.
...
2. `docs/development/TESTING.md` incorpora la suite focal de Productos, la sintaxis correcta de Vitest y el control de tipos discriminados.
3. `docs/development/FRONTEND_DEBUG.md` incorpora el preflight de Chrome, el alcance real de una verificación HTTP y el manejo de auditorías npm bajo sandbox.
4. `../features/API_PRODUCTS.md`, `../features/PRODUCTS_UI.md` y `../features/roles-endpoints.md` distinguen preview legado, preview batch Vue y asignación definitiva.
5. `docs/development/DEVELOPMENT_WORKFLOW.md` y README vuelven a señalar `scripts/start-dev.ps1` y Vue 5176 como inicio canónico; `start.bat`/React 5173 queda identificado como heredado.
