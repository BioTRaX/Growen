...
<!-- NG-HEADER: Ubicación: docs/retrospectives/RETROSPECTIVE_TELEGRAM_SECRET_FORENSICS_20260815.md -->
...
| El primer escaneo remoto todavía alcanzó el blob antiguo | GitHub conserva referencias internas `refs/pull/*` aunque se reescriban heads y tags | Se enumeraron diez referencias y se escaló la purga; la confirmación operativa posterior la da por finalizada | Resuelto |
...
| La documentación afirmó posteriormente que la clave OpenAI local había sido eliminada | Contradicción entre `docs/operations/SECURITY.md` y el informe forense | El 2026-08-15 se verificó sin mostrar valores que continúa una coincidencia real en `.env`, ignorada y no rastreada; la política fue corregida | Rotación pendiente |
...
- Las diez referencias `refs/pull/*` y los objetos asociados figuraban como
  deuda al cerrar esta retrospectiva. La confirmación operativa posterior da la
  purga y la invalidación de copias afectadas por finalizadas; no quedan acciones
  pendientes del incidente Telegram.
...
- `scripts/check-quality.ps1 -SkillsOnly -SkillName <nombre>` valida focalmente
  ...
  existente.
- Se corrigió `docs/operations/SECURITY.md` para reflejar el estado real de la clave local.
