...
## Actualización 2026-08-27 — adaptación de Superpowers a Growen

- [x] Definida la precedencia de `AGENTS.md` y las skills canónicas de Growen sobre las 14 skills globales de Superpowers.
...
## Actualización 2026-08-20 — calidad editorial y legibilidad de Enrich v2

- [x] Descripciones Enrich redactadas como texto directo y publicable, sin referencias a fuentes o investigación.
- [x] Especificaciones e instrucciones estructuradas con etiquetas legibles, listas y booleanos en lenguaje natural.
- [x] Centro de Conocimiento de Producto migrado a la vista dedicada `/productos/:id/Conocimiento`; Mercado conserva el diálogo contextual sobre el mismo componente compartido.
- [x] Retrospectiva persistida y `vue-module-migration` ampliada con elección de superficie y smoke visual autenticado.
- [ ] Reejecutar Enrich sobre productos con contenido anterior para regenerarlos bajo el nuevo contrato editorial.
- [ ] Ejecutar primero un lote masivo pequeño y verificar deduplicación, estados terminales, calidad editorial y volumen de activos.
- [ ] Documentar los cambios y actualizar cualquier referencia que resulte desactualizada durante el próximo smoke real.

## Actualización 2026-08-19 — GPT-5.6 Luna para Enrich

- [x] Migrado el modelo remoto de Enrich a `gpt-5.6-luna` sin modificar Chat ni
  el modelo explícito de Conocimiento.
- [x] Definidos razonamiento `none`, temperatura `0`, JSON y presupuesto de
  salida de 2048 tokens para conservar determinismo y costo acotado.
- [x] Eliminados los reintentos del SDK y los reintentos Dramatiq ante errores
  permanentes de crédito, cuota o clave.
- [x] Reconstruido y recreado `enrichment_worker`; health, broker, heartbeat,
  modelo y archivo de secreto quedaron operativos.
- [ ] Agregar créditos a la organización asociada a la API key y repetir el
  smoke real hasta obtener una propuesta estructurada válida.
- [ ] Documentar el resultado del próximo smoke y actualizar cualquier
  instrucción que resulte desactualizada durante el despliegue.

## Actualización 2026-08-17 — observabilidad de proveedores Enrich

- [x] Reemplazados los errores indistinguibles de proveedor por diagnósticos persistidos y acotados por intento.
- [x] Incorporados eventos `provider_started`, `provider_succeeded` y `provider_failed` con correlación propia, sin prompts ni secretos.
- [x] OpenAI conserva `error.code`, HTTP, `x-request-id` y headers de límites/resets; Ollama distingue timeout, HTTP, respuesta vacía, JSON y esquema inválidos.
- [x] La ficha Vue recupera el último job y muestra su diagnóstico técnico para staff.
- [ ] Repetir el smoke real con un job nuevo para registrar la causa exacta del 429 vigente y validar un resultado estructurado exitoso.

## Actualización 2026-08-17 — aislamiento de secretos por worker

- [x] Extraída de `Settings.__post_init__()` la validación específica de Telegram; `telegram_worker` la ejecuta como preflight de su propio runtime.
- [x] Aislados `dramatiq`, Mercado, Enrich y Conocimiento con flags Telegram apagados y sin acceso al token del bot.
- [x] Limitado el secreto OpenAI a Enrich y Conocimiento mediante bind mount de sólo lectura; Mercado y Dramatiq no lo reciben.
- [x] Reconstruidos y recreados los workers; heartbeats reales de Enrich, Conocimiento y Mercado verificados saludables.
- [x] Agregada cobertura para impedir que workers ajenos vuelvan a heredar secretos Telegram desde `.env`.
- [ ] Resolver la respuesta HTTP 429 del proveedor OpenAI; Ollama está operativo, pero debe producir un esquema Enrich válido antes de aprobar el smoke end-to-end.
- [ ] Generalizar el patrón a configuraciones tipadas por dominio para reducir futuras dependencias por importación.
...
### 2026-08-27 — limpieza legacy y convergencia pendiente

- [x] Verificar estado Alembic y confirmar ausencia de `market_sources`.
- [x] Retirar bloques inalcanzables de los adaptadores de enriquecimiento.
- [x] Migrar el batch operativo Vue a `/canonical-products/enrichment-batches`.
- [x] Diferir la carga opcional de `rembg` para aislar pruebas y arranque de API
  de la cadena multimedia pesada.
- [ ] Migrar los consumidores React restantes a los endpoints canónicos antes de
  retirar los adaptadores públicos.
- [ ] Eliminar campos legacy de `Product` mediante una migración separada,
  después del ciclo estable de rollback React.
- [ ] Sustituir referencias internas a `MarketSource` por
  `CanonicalKnowledgeMarketProfile` y retirar el alias cuando no existan
  consumidores.
