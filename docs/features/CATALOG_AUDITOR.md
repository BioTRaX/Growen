...
La autenticación del navegador aislado no estaba disponible para el smoke, por
lo que el run real se creó con una sesión HTTP admin y CSRF contra el mismo
endpoint que consume Vue. El componente Vue sí tiene prueba de interacción y
ahora ofrece **Reanudar fallidos** también cuando el run terminó como
`completed_with_issues`. Sigue pendiente repetir el clic inicial desde una
sesión Vue autenticada antes de cerrar por completo el gate de esta fase.
La identificación legible y la aceptación desde Productos están implementadas
y cubiertas por pruebas automatizadas, pero su smoke autenticado sobre los 29
canónicos continúa pendiente; este estado no equivale a producción verificada.
