...
<!-- NG-HEADER: Ubicación: docs/features/MIGRATIONS_NOTES.md -->
...
`users.is_active BOOLEAN NOT NULL DEFAULT true` para permitir desactivar una
cuenta e invalidar todas sus sesiones sin eliminar su historial. El downgrade
aborta si existen usuarios inactivos, evitando reactivar cuentas de forma
silenciosa, y luego elimina la columna.

Antes del despliegue ejecutar `alembic heads`, probar upgrade desde el head
anterior y desde una PostgreSQL vacía, y verificar login/invalidez de sesiones.
Esta entrega no aplicó la revisión sobre ninguna base operativa.
...
- Opcional: `DISABLE_AUTO_BACKUP=1` en `.env` si no se desea backup automático en dev (ver `docs/operations/BACKUPS.md`).
