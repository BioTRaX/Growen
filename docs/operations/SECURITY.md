...
<!-- NG-HEADER: Ubicación: docs/operations/SECURITY.md -->
...
`meli_cloudflared` sólo está conectado a `meli_ingress` y `cloudflare_egress`; la última regla ingress es `http_status:404`. La configuración no permite alcanzar otros servicios Docker. Ver rotación y operación en `docs/features/MELI_INTEGRATION.md`.
...
- El incidente fue declarado cerrado por confirmación operativa: las referencias
  internas afectadas y los objetos asociados ya fueron purgados y no quedan
  acciones pendientes de erradicación. Esta actualización de estado no equivale
  a una nueva inspección remota desde esta sesión.
...
- Informe, alcance y plan de erradicación:
  `docs/retrospectives/SECURITY_INCIDENT_TELEGRAM_20260730.md`.
...
|------|--------|-------|----------|
| 1 | Ejecutar `.\.venv\Scripts\python.exe scripts/check_pdf_crypto.py C:\GrowenPrivate\purchases --recursive --json` | Sin PDFs ARC4 | N/A |
