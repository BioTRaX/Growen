...
class Product(Base):
    ...
    # Metadatos de trazabilidad del enriquecimiento
    last_enriched_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    enriched_by: Mapped[Optional[int]] = mapped_column(ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    # Flag para evitar enriquecimientos concurrentes
    is_enriching: Mapped[bool] = mapped_column(Boolean, default=False, server_default='false', nullable=False)
    # Datos técnicos editables desde UI (medidas y precio de referencia de mercado)
