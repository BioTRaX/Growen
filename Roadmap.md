...
- [x] Gestor integral de imágenes de productos y sincronización con Google Drive (`/imagenes-productos` y `/productos/:id/imagenes`): selector de carpeta Drive (ID por defecto `13d0sHLN0LrKAuxBV-Aibxrq05jz0n8F7` "Galeria x SKU"), contraste de SKU canónico con previsualización (`/admin/drive-sync/preview`), soporte de fotos adicionales en la misma galería del producto, prevención de re-descargas (idempotencia), soporte de formatos HEIC, PNG y JPG con derivadas WebP para navegadores, suite de edición individual interactiva (`Cropper`, rotación ±90°, recorte cuadrado con margen) y procesamiento masivo con Logo Institucional PNG (2026-09-20).
